...
void crcencoding::verifierCRC()
{
	...
	uint8_t verification = GetRemainderCRC(fCodeword, fGenerator);

	//check whether or not the remainder is zero
	if (verification)
	{//if the remainder is nonzero, an error has occured in the bit stream
		std::cout << "The bit stream contains an error. Please retransmit.\n" << std::endl;
	}
	else
	{//if the remainder is zero, the message was transmitted with no detectable errors
		std::cout << "The bit stream was transmitted with no detectable errors.\n" << std::endl;
	}
}

uint8_t crcencoding::alterCRC(uint8_t const bitnumber)
{
	//placement of the bit error
	uint8_t codewordbits = (int)log2(fCodeword) + 1;
	uint8_t biterror = 1 << (codewordbits - bitnumber);

	//XOR the message with the bit error to flip the desired bit and create an incorrect bit stream
	//uint8_t corruptcodeword = codeword^biterror;
	fCodeword ^= biterror;

	std::cout << "Bit error: " << std::bitset<8>(biterror) << std::endl;
	std::cout << "Codeword: " << std::bitset<8>(fCodeword) << std::endl;

	return fCodeword;
}

uint8_t crcencoding::GetRemainderCRC(uint8_t const dividend, uint8_t const generator)
{
	uint8_t rcq; //Shift register and output contents
	...
	//std::cout << sizedif << std::endl;

	rcq = dividend;
	//shift = generator<<(gendeg);

	for (int i = 0; i <= sizedif; i++) //log2(POLYNOMIAL)
	{
		//std::cout << std::bitset<8>(rcq) << std::endl;
		//std::cout << std::bitset<8>(generator << (sizedif - i)) << std::endl;
		//std::cout << "--------" << std::endl;
		...
		//Shift the generator down by 1 bit
		//shift = (shift >> 1);
	}
